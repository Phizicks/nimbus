...
LIFECYCLE_ENDPOINT = os.getenv('LIFECYCLE_ENDPOINT_URL', 'http://lambda:4566')


def lambda_request(method: str, path: str, **kwargs):
    url = LIFECYCLE_ENDPOINT.rstrip('/') + path
    try:
...
FUNCTIONS_DIR = Path('/tmp/lambda-functions')
...
def put_image():
    ...
    try:
        ...
        local_image.tag(f"{REGISTRY_HOST}/{repo_path}", tag=image_tag)
        docker_client.images.push(f"{REGISTRY_HOST}/{repo_path}", tag=image_tag)
...
def list_images():
    ...
    try:
        ...
        registry_url = f"http://{REGISTRY_HOST}/v2/{repo_path}/tags/list"
        ...
        for tag in tags:
            try:
                ...
                manifest_url = f"http://{REGISTRY_HOST}/v2/{repo_path}/manifests/{tag}"
...
def docker_registry_root():
    ...
    try:
        ...
        resp = requests.get(f"http://{REGISTRY_HOST}/v2/")
...
def docker_registry_proxy(path):
    ...
    try:
        ...
        url = f"http://{REGISTRY_HOST}/v2/{path}"
            ...
            if k.lower() not in excluded:
                ...
                if k.lower() == 'location':
                    if v.startswith('http://') or v.startswith('https://'):
                        # Parse and rewrite
                        original_host = f"http://{REGISTRY_HOST}"
...
def complete_layer_upload():
    ...
    image.tag(f"{REGISTRY_HOST}/{repo_path}", tag='latest')
    docker_client.images.push(f"{REGISTRY_HOST}/{repo_path}", tag='latest')
...
def put_image():
    ...
    try:
        manifest_url = f"http://{REGISTRY_HOST}/v2/{repo_path}/manifests/{image_tag}"
        manifest_response = requests.get(
            manifest_url,
            headers={'Accept': 'application/vnd.docker.distribution.manifest.v2+json'}
        )
