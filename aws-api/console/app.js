...
async function purgeQueue(queueUrl, queueName) {
    if (!confirm(`Are you sure you want to purge all messages from ${queueName}?`)) {
        return;
    }
...
async function deleteQueue(queueUrl, queueName) {
    if (!confirm(`Are you sure you want to delete ${queueName}? This cannot be undone.`)) {
        return;
    }
...
async function deleteESM(uuid) {
    if (!confirm('Are you sure you want to delete this event source mapping?')) {
        return;
    }
...
async function deleteSelectedLogGroups() {
    ...
    if (!confirm(`Are you sure you want to delete ${logGroups.length} log group(s)?`)) {
        return;
    }
...
async function deleteLogGroup(logGroupName) {
    if (!confirm(`Are you sure you want to delete log group "${logGroupName}"?`)) {
        return;
    }
...
async function deleteSelectedLogStreams(logGroupName) {
    ...
    if (!confirm(`Are you sure you want to delete ${logStreams.length} log stream(s)?`)) {
        return;
    }
...
async function deleteLogStream(logGroupName, logStreamName) {
    if (!confirm(`Are you sure you want to delete log stream "${logStreamName}"?`)) {
        return;
    }
...
async function deleteImage(repositoryName, imageDigest, imageTag) {
    ...
    if (!confirm(`Are you sure you want to delete this image${tagInfo} from ${repositoryName}?`)) {
        return;
    }
...
async function deleteRepository(repositoryName, hasImages) {
    ...
    if (!confirm(`Are you sure you want to delete repository ${repositoryName}?${forceWarning}\n\nThis cannot be undone.`)) {
        return;
    }
