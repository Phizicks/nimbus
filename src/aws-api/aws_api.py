...
FUNCTIONS_DIR = Path(
    "/tmp/lambda-functions"
)  # Yep cheating, until S3 and ECR fully supporting
FUNCTIONS_DIR.mkdir(exist_ok=True)

# MinIO ARN mapping for webhook targets configured via env vars
_MINIO_WEBHOOK_ARNS = {
    "Queue":  "arn:minio:sqs::SQS:webhook",
    "Topic":  "arn:minio:sqs::SNS:webhook",
    "Lambda": "arn:minio:sqs::LAMBDA:webhook",
}
# Maps config_type to the boto3 NotificationConfiguration key and its ARN field
_NOTIFICATION_CONFIG_MAP = {
...
def proxy_to_dynamodb(operation: str, data=None):
    ...
    status, resp, raw = dynamodb_request(
        "POST", "/", headers=headers, json=(data or {})
    )
    if raw is None:
        return jsonify(resp), status
...
def proxy_logs_to_lambda(operation: str, data=None):
    ...
    try:
        return Response(
            raw.content,
            status=raw.status_code,
            mimetype=raw.headers.get("Content-Type", "application/json"),
        )
    except Exception:
        return jsonify(resp), status
...
def get_request_data():
    """Get data from request, handling JSON, form-encoded, and XML (AWS S3) protocols"""
...
_RESOURCE_SEP_RE = re.compile(
    r"/(?:blobs|manifests|uploads|_uploads|tags|_layers|_catalog|_manifests)(?:/|$)",
    re.IGNORECASE,
)
...
def ecr_repository_exists(repository_name):
    ...
    cursor.execute(
        "SELECT * FROM repositories WHERE repository_name = ?", (repository_name,)
    )
    if cursor.fetchone():
        conn.close()
        return True
...
def describe_repositories():
    ...
    if repository_names:
        ...
        if missing_repos:
            ...
            raise ValueError(
                f"The repository(s) {', '.join(missing_repos)} does not exist"
            )
    else:
...
def delete_repository():
                ...
                for tag in tags:
                    ...
                    if manifest_resp.status_code == 200:
                        ...
                        if digest:
                            ...
                            delete_url = (
                                f"{registry_url}/v2/{repo_path}/manifests/{digest}"
                            )
                            delete_resp = requests.delete(delete_url, timeout=5)

                            if delete_resp.status_code in (202, 204):
                                logger.info(
                                    f"Deleted {repo_path}:{tag} (digest: {digest})"
                                )
                            else:
                                logger.warning(
                                    f"Failed to delete {tag}: status {delete_resp.status_code}"
                                )
                    else:
                        logger.warning(
                            f"Failed to get manifest for {tag}: {manifest_resp.status_code}"
                        )

            elif tags_resp.status_code != 404:
                logger.error(
                    f"Failed to list tags for {repo_path}: {tags_resp.status_code} - {tags_resp.text}"
                )
    ...
    cursor.execute(
        "DELETE FROM repositories WHERE repository_name = ?", (repository_name,)
    )

    conn.commit()
    conn.close()

    return {"repository": {"repositoryName": repository_name, "registryId": ACCOUNT_ID}}
    ...
    try:
        ...
        for tag in tags:
            try:
                ...
                manifest_url = (
                    f"http://{BACKEND_REGISTRY_HOST}/v2/{repo_path}/manifests/{tag}"
                )
                ...
                if manifest_response.status_code == 200:
                    digest = manifest_response.headers.get(
...
def describe_images():
    ...
    try:
        ...
        if image_ids:
            requested_tags = [
                img.get("imageTag") for img in image_ids if "imageTag" in img
            ]
            if requested_tags:
        ...
        for tag in tags:
            try:
                ...
                logger.warning(
                    f"Error getting details for {repository_name}:{tag}: {e}"
                )
                continue
...
def batch_delete_image():
    ...
    for img_id in image_ids:
        ...
        try:
            ...
            if image_tag:
                cursor.execute(
                    "DELETE FROM images WHERE repository_name = ? AND image_tag = ?",
                    (repository_name, image_tag),
                )
            elif image_digest:
                cursor.execute(
                    "DELETE FROM images WHERE repository_name = ? AND image_digest = ?",
...
def initiate_layer_upload():
    ...
    return {"uploadId": upload_id, "partSize": 5242880}  # 5MB chunks
...
def put_image():
    ...
    try:
        ...
        raise ValueError(
            f"Local image {repository_name}:{image_tag} not found for push"
        )

    # Get actual digest from registry after push
    try:
        manifest_url = (
            f"http://{BACKEND_REGISTRY_HOST}/v2/{repo_path}/manifests/{image_tag}"
        )
        manifest_response = requests.get(
            manifest_url,
            headers={"Accept": "application/vnd.docker.distribution.manifest.v2+json"},
        )
        image_digest = manifest_response.headers.get(
            "Docker-Content-Digest", "sha256:unknown"
        )
    except Exception as e:
...
def batch_get_image():
    ...
    for img_id in image_ids:
        ...
        try:
            ...
            digest = manifest_resp.headers.get(
                "Docker-Content-Digest", "sha256:unknown"
            )
    ...
    def validate_request(self, request):
        ...
        try:
            ...
            if signature == expected_sig:
                ...
                logger.warning(
                    f"Signature mismatch. Expected: {expected_sig}, Got: {signature}"
                )
                # In LocalStack mode, log warning but don't reject
                if self.skip_validation:
                    logger.warning(
                        "Signatures do not match, but accepting due to skip_validation=True"
                    )
...
def runtime_invocation_response(request_id):
    ...
    headers = dict(request.headers)
    headers["X-Forwarded-For"] = client_ip
    status, _, raw = lambda_request(
        "POST",
        f"/2018-06-01/runtime/invocation/{request_id}/response",
        headers=headers,
        data=response_data,
    )
...
def runtime_invocation_error(request_id):
    ...
    status, _, raw = lambda_request(
        "POST",
        f"/2018-06-01/runtime/invocation/{request_id}/error",
        headers=headers,
        data=error_data,
    )
    if status in (200, 202, 204):
...
def runtime_init_error():
    ...
    error_data = request.get_data()
    headers = dict(request.headers)
    status, body, raw = lambda_request(
        "POST", f"/2018-06-01/runtime/init/error", headers=headers, data=error_data
    )
    return Response(
        body,
        status=status,
...
def delete_function(function_name):
    ...
    try:
        ...
        status, resp, raw = lambda_request(
            "DELETE", f"/2015-03-31/functions/{function_name}"
        )
...
def update_function_code(function_name):
    ...
    try:
        ...
        # Proxy to lambda service endpoint
        status, resp, raw = lambda_request(
            "PUT", f"/2015-03-31/functions/{function_name}/code", json=data
        )
...
def get_function_configuration(function_name):
    ...
    try:
        ...
        # Proxy to lambda service endpoint
        status, resp, raw = lambda_request(
            "GET", f"/2015-03-31/functions/{function_name}/configuration"
        )
...
def update_function_configuration(function_name):
    ...
    try:
        ...
        # Proxy to lambda service endpoint
        status, resp, raw = lambda_request(
            "PUT", f"/2015-03-31/functions/{function_name}/configuration", json=data
        )
...
def webhook_s3():
    ...
    try:
        ...
        for record in event_data.get("Records", []):
            ...
            if not configs:
                logger.warning(
                    f"No notification configs found for bucket {bucket_name}"
                )
                continue
            ...
            if not matched_queues:
                logger.warning(
                    f"Event {event_name} on {object_key} matched no notification configs"
                )
...
# Lambda Webhook
@app.route("/webhook/lambda", methods=["POST"])
def webhook_lambda():
    ...
    try:
        ...
        for record in event_data.get("Records", []):
            ...
            for config in configs:
                # Only process Lambda-type configs (queue_url holds real Lambda ARN)
                queue_url = config.get("queue_url", "")
                if ":lambda:" not in queue_url and "function:" not in queue_url:
                    continue
...
def send_event_to_lambda(function_name, record: dict):
    """Send S3 event record to Lambda function"""
    try:
...
def sqs_status():
    ...
    try:
        ...
        logger.error(
            f"Failed to fetch SQS status from {aws_sqs_url}: {e}", exc_info=True
        )
...
def invoke_function(function_name):
    ...
    try:
        status, resp, raw = lambda_request(
            "POST",
            f"/2015-03-31/functions/{function_name}/invocations",
            headers=headers,
            data=raw_payload,
        )
        ...
        if raw is None:
            logger.error(
                f"No response received from lambda service for {function_name}"
            )
            return (
                jsonify(
                    {
        ...
        logger.info(
            f"Returning response for {function_name}: status={status}, content_type={content_type}"
        )

        # Return the response exactly as received from lambda service
        return Response(
            raw.content, status=status, headers=response_headers, mimetype=content_type
        )
...
@app.route(
    "/2015-03-31/functions/<function_name>/provisioned-concurrency-configs",
    methods=["PUT"],
)
...
def delete_provisioned__concurrency(function_name):
    ...
    try:
        ...
        logger.error(
            f"Error deleting concurrency for {function_name}: {e}", exc_info=True
        )
        error_response = {"__type": "ServiceException:", "message": str(e)}
        return error_response, 500
...
def cloudwatch_logs_api():
    ...
    status, data, resp = log_request(
        "POST", "/logs", headers=proxied_headers, data=raw_body
    )
...
def put_function_logging_config(function_name):
    ...
    try:
        ...
        status, resp, raw = log_request(
            "PUT", f"/2015-03-31/functions/{function_name}/logging-config", json=data
        )
...
def get_function_logging_config(function_name):
    ...
    try:
        ...
        # Proxy to lambda service endpoint
        status, resp, raw = log_request(
            "GET", f"/2015-03-31/functions/{function_name}/logging-config"
        )
...
def delete_function_logging_config(function_name):
    ...
    try:
        ...
        # Proxy to lambda service endpoint
        status, resp, raw = log_request(
            "DELETE", f"/2015-03-31/functions/{function_name}/logging-config"
        )
...
@app.route(
    "/", methods=["POST", "GET", "PUT", "DELETE", "PATCH", "HEAD"], strict_slashes=False
)
def handle_request():
    ...
    if action in SQS_ACTIONS:
        ...
        try:
            resp = requests.request(
                request.method, url, headers=forward_headers, data=body, timeout=300
            )
        except requests.exceptions.ConnectionError as e:
            logger.error(
                f"Failed to connect to SQS container at {AWS_ENDPOINT_URL_SQS}: {e}"
            )
            return (
                jsonify(
                    {
            ...
            logger.error(
                f"Error proxying SQS API request to container: {e}", exc_info=True
            )
            return jsonify({"__type": "ServiceException", "message": str(e)}), 500
...
def handle_bucket_operations(bucket_name):
    """Handle bucket-level PUT operations (notifications, ACLs, etc.)."""

    if "notification" not in request.args:
        return proxy_to_s3()
    ...
    notification_types = {
        ...
        "QueueConfigurations":            ("QueueArn",            "sqs"),
        "TopicConfigurations":            ("TopicArn",            "sns"),
        "LambdaFunctionConfigurations":   ("LambdaFunctionArn",   "lambda"),
        # Legacy AWS CLI XML element names
        "QueueConfiguration":             ("Queue",               "sqs"),
        "TopicConfiguration":             ("Topic",               "sns"),
    ...
    for config_key, (arn_field, target_type) in notification_types.items():
        ...
        for config in configs:
            ...
            notification_id = config.get("Id", f"{config_key}-{int(time.time() * 1000)}")
            ...
            logger.info(
                f"Saved {target_type} notification {notification_id} for bucket {bucket_name} -> {arn}"
            )
            configs_found = True
...
#     topic_arn = config.get("TopicArn") or config.get("Topic")
#     lambda_arn = (
#         config.get("LambdaFunctionArn")
#         or config.get("CloudFunction")
#         or config.get("LambdaFunction")
#     )
...
#     if queue_arn:
#         target_type = "sqs"
#         target_name = queue_arn.split(":")[-1]
#         target_url = f"http://sqs:4566/{ACCOUNT_ID}/{target_name}"
#     elif topic_arn:
#         target_type = "sns"
#         target_name = topic_arn.split(":")[-1]
#         target_url = f"http://sns:4566/{ACCOUNT_ID}/{target_name}"
#     else:
#         target_type = "lambda"
#         target_name = lambda_arn.split(":")[-1]
#         target_url = lambda_arn  # just store the ARN directly for Lambda
...
#     logger.info(
#         f"Saved {target_type} notification config {notification_id} for bucket {bucket_name}"
#     )


@app.route("/probe-bsign-<path:random>", methods=["GET"])
def s3_probe(random):
    xml = f"""<?xml version="1.0" encoding="UTF-8"?>
<Error>
  <Code>NoSuchBucket</Code>
  <Message>The specified bucket does not exist</Message>
  <BucketName>probe-bsign-{random}</BucketName>
  <RequestId>0000000000000000</RequestId>
  <HostId>00000000000000000000000000000000</HostId>
</Error>"""
    return Response(xml, status=404, content_type="application/xml")
...
def handle_bucket_get_operations(bucket_name):
    ...
    # Group configs by type based on ARN prefix
    queue_configs = []
    topic_configs = []
    lambda_configs = []
    ...
    for config in configs:
        events = (
            json.loads(config["event_patterns"])
            if isinstance(config["event_patterns"], str)
            else config["event_patterns"]
        )
        filter_rules = (
            json.loads(config["filter_rules"])
            if config.get("filter_rules") and isinstance(config["filter_rules"], str)
            else []
        )

        arn = config["queue_arn"]  # stored generically regardless of type
        notification_id = config["notification_id"]

        filter_xml = ""
        if filter_rules:
            rules_xml = "".join(
                f"<FilterRule><Name>{r['Name']}</Name><Value>{r['Value']}</Value></FilterRule>"
                for r in filter_rules
            )
            filter_xml = f"<Filter><S3Key>{rules_xml}</S3Key></Filter>"
        ...
        if ":sqs:" in arn:
            queue_configs.append(
                ...
                f"<Id>{notification_id}</Id>"
                f"<Queue>{arn}</Queue>"
                f"{events_xml}{filter_xml}"
                f"</QueueConfiguration>"
            )
        elif ":sns:" in arn:
            ...
            lambda_configs.append(
                ...
                f"<Id>{notification_id}</Id>"
                f"<CloudFunction>{arn}</CloudFunction>"
                f"{events_xml}{filter_xml}"
                f"</CloudFunctionConfiguration>"
            )
        else:
            # Fallback: treat as queue (legacy)
            queue_configs.append(
                f"<QueueConfiguration>"
                f"<Id>{notification_id}</Id>"
                f"<Queue>{arn}</Queue>"
    ...
    try:
        ...
        for config in configs:
            ...
            filter_rules = (
                ...
                and isinstance(config["filter_rules"], str)
                else []
            )
            ...
            if filter_rules:
                queue_config["Filter"] = {"Key": {"FilterRules": filter_rules}}
...
def configure_bucket_notification_webhook(
    bucket_name, notification_id, config_type, queue_arn, events=[], filters=[]
):
    s3 = boto3.client(
        "s3",
    ...
    # config_id = f"webhook-{notification_id}"
    ...
    current = s3.get_bucket_notification_configuration(Bucket=bucket_name)
    current_configs: list = current.get("QueueConfigurations", [])
    if current_configs:
        for i, cfg in enumerate(current_configs):
            if cfg.get("Id") == notification_id:
                current_configs[i] = new_config
                # current_configs.append(new_config)
                break
        else:
            # Existing entry to replace was not found, so we append it
            current_configs.append(new_config)

    save_config: dict = {}
    save_config["QueueConfigurations"] = current_configs
    s3.put_bucket_notification_configuration(
        Bucket=bucket_name,
        NotificationConfiguration=save_config,
    )


def _configure_minio_notification(bucket_name, config_type, notification_id, events, filter_config):
    """
    Register or replace a single notification entry in MinIO.
    All three types are stored as QueueConfigurations since MinIO only supports queue-based webhooks.
    """
    minio_arn = _MINIO_WEBHOOK_ARNS.get(config_type)
    if not minio_arn:
    ...
    if filter_config:
        new_entry["Filter"] = filter_config

    s3 = get_s3_client()
    current = s3.get_bucket_notification_configuration(Bucket=bucket_name)
    current.pop("ResponseMetadata", None)

    existing_list: list = current.get("QueueConfigurations", [])
...
def put_bucket_notifications(bucket_name):
    """Add or replace a single bucket notification config (web console)."""
    try:
        data = request.get_json() or {}

        config_type = data.get("type")   # "Queue" | "Topic" | "Lambda"
        config = data.get("configuration")

        if not config_type or not config:
            return jsonify({
                "__type": "InvalidParameterException",
                "message": "Missing 'type' or 'configuration' in request",
            }), 400

        if config_type not in _NOTIFICATION_CONFIG_MAP:
            return jsonify({
                "__type": "InvalidParameterException",
                "message": f"Invalid type '{config_type}'. Must be Queue, Topic, or Lambda",
            }), 400

        _, arn_field = _NOTIFICATION_CONFIG_MAP[config_type]
        user_arn = config.get(arn_field)
        notification_id = config.get("Id")
        events = config.get("Events", [])
        filter_config = config.get("Filter", {})

        if not user_arn or not notification_id:
            return jsonify({
                "__type": "InvalidParameterException",
                "message": f"{arn_field} and Id are required in configuration",
            }), 400

        filter_rules = []
        if filter_config:
            filter_rules = filter_config.get("Key", {}).get("FilterRules", [])

        # Derive queue_url for SQS routing; for SNS/Lambda store ARN directly
        if config_type == "Queue":
            target_name = user_arn.split(":")[-1]
            queue_url = f"http://sqs:4566/{ACCOUNT_ID}/{target_name}"
        else:
            queue_url = user_arn

        # 1. Save/replace in DB (source of truth for webhook routing)
        db.save_notification_config(
            bucket_name=bucket_name,
            notification_id=notification_id,
            queue_arn=user_arn,
        ...
        # 2. Register/replace in MinIO (triggers the webhook fire)
        _configure_minio_notification(
...
@app.route(
    "/s3/buckets/<bucket_name>/notification/<notification_id>", methods=["DELETE"]
)
...
def add_bucket_lifecycle_rule(bucket_name):
    ...
    try:
        ...
        if not new_rule:
            return (
                jsonify(
                    {"__type": "InvalidParameterException", "message": "Missing rule"}
                ),
                400,
            )
        ...
        if any(r.get("ID") == rule_id for r in existing_rules):
            return (
                jsonify(
                    {
                        "__type": "InvalidParameterException",
                        "message": f"Rule ID {rule_id} exists",
                    }
                ),
                400,
            )
...
def delete_s3_bucket(bucket_name):
    ...
    try:
        ...
        return (
            jsonify({"__type": "NoSuchBucket", "message": "Bucket does not exist"}),
            404,
        )
    except Exception as e:
        error_message = str(e)
        ...
        if not all([source_key, dest_bucket, dest_key]):
            return (
                jsonify(
                    {
                        "__type": "InvalidParameterException",
                        "message": "sourceKey, destBucket, destKey required",
                    }
                ),
                400,
            )
...
def list_event_source_mapping_endpoint():
    ...
    status, resp = esm_request(
        "GET",
        (
            f"/2015-03-31/event-source-mappings/?FunctionName={function_name}"
            if function_name
            else "/2015-03-31/event-source-mappings/"
        ),
    )
    return (jsonify(resp), status) if isinstance(resp, dict) else (resp, status)
...
@app.route(
    "/2015-03-31/event-source-mappings/<uuid>", methods=["GET"], strict_slashes=False
)
...
@app.route(
    "/2015-03-31/event-source-mappings/<uuid>", methods=["PUT"], strict_slashes=False
)
def update_event_source_mapping_endpoint(uuid):
    """Update event source mapping"""
    data = request.get_json() or {}
    status, resp = esm_request(
        "PATCH", f"/2015-03-31/event-source-mappings/{uuid}", json=data
    )
    if status == 200:
        return jsonify(resp)
    return jsonify({"__type": "ResourceNotFoundException"}), 404
...
@app.route(
    "/2015-03-31/event-source-mappings/<uuid>", methods=["DELETE"], strict_slashes=False
)
