...
class CloudWatchLogsDatabase:
    ...
    def delete_log_group(self, log_group_name):
        ...
        cursor.execute('DELETE FROM log_groups WHERE log_group_name = ?', (log_group_name,))
    ...
    def log_group_exists(self, log_group_name: str):
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT 1 FROM log_groups WHERE log_group_name = ?', (log_group_name,))
    ...
    def create_log_stream(self, log_group_name, log_stream_name):
        ...
        try:
            ...
            logger.debug(f"Log stream already exists: {log_group_name}/{log_stream_name}")
            return False
        finally:
            conn.close()
    ...
    def list_log_streams(self, log_group_name, prefix=None, limit=50, order_by='LogStreamName'):
        ...
        order_clause = 'log_stream_name' if order_by == 'LogStreamName' else 'last_event_timestamp DESC'
    ...
    def put_log_events(self, log_group_name, log_stream_name, events):
        ...
        for event in events:
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?)
            ''', (log_group_name, log_stream_name, event['timestamp'], ingestion_time, event['message']))
        ...
        cursor.execute('''
            ...
            WHERE log_group_name = ? AND log_stream_name = ?
        ''', (log_group_name, log_stream_name, log_group_name, log_stream_name,
              ingestion_time, log_group_name, log_stream_name, log_group_name, log_stream_name))
    ...
    def get_log_events(self, log_group_name, log_stream_name, start_time=None,
                       end_time=None, limit=10000, start_from_head=True):
    ...
    def filter_log_events(self, log_group_name, log_stream_names=None, start_time=None,
                         end_time=None, filter_pattern=None, limit=10000):
...
class LogManager:
    ...
    def __init__(self, docker_client, max_logs_per_invocation=1000, log_retention_seconds=300):
        ...
        self.invocation_logs = defaultdict(lambda: deque(maxlen=max_logs_per_invocation))
    ...
    def start_container_logging(self, container_id, function_name, log_group_name=None, log_stream_name=None):
        """
        Start capturing logs from a container and send to CloudWatch.
        """
        logger.info(f"start_container_logging called: container_id={container_id}, log_group={log_group_name}")
        ...
        with self._lock("LogManager.start_container_logging"):
            ...
            if log_group_name and log_stream_name:
                ...
                logger.info(f"Stored log config for {container_id}: {log_group_name}/{log_stream_name}")
            ...
            logger.info(f"Started log streaming thread for container {container_id} -> {log_group_name}/{log_stream_name}")
    ...
    def write_start_line(self, request_id, log_group, log_stream):
        ...
        try:
            ...
            self.put_log_events(log_group, log_stream, [{
                'timestamp': timestamp,
                'message': start_line
            }])
            logger.debug(f"Added START line to CloudWatch for {C.CYAN}{request_id}{C.RESET}")
    ...
    def write_end_line(self, request_id, log_group, log_stream):
        ...
        try:
            ...
            logger.debug(f"Added END line to CloudWatch for {C.CYAN}{request_id}{C.RESET}")
        except Exception as e:
    ...
    def _stream_container_logs_safe(self, container_id, function_name):
        ...
        try:
            ...
            logger.error(f"Exception in log streaming thread for {container_id}: {e}", exc_info=True)
    ...
    def _stream_container_logs(self, container_id, function_name):
        ...
        """
        try:
            logger.debug(f"[{time.time()}] _stream_container_logs STARTED for {container_id}")
            ...
            logger.debug(f"CloudWatch config: log_group={log_group}, log_stream={log_stream}")
            ...
            log_config_driver = container.attrs.get('HostConfig', {}).get('LogConfig', {})
            ...
            for log_line in log_stream_obj:
                try:
                    ...
                    if active_request != current_request_id:
                        ...
                        if active_request:
                            logger.debug(f"Container {container_id} now processing request {C.CYAN}{active_request}{C.RESET}")
                    ...
                    logger.debug(f"[Container:{container_id}][Request:{C.CYAN}{current_request_id}{C.RESET}] {message[:100]}")

                    # Flush CloudWatch buffer if needed
                    now = time.time()
                    should_flush = (
                        len(cloudwatch_buffer) >= MAX_BATCH_SIZE or
                    ...
                    if should_flush and log_group and log_stream:
                        try:
                            ...
                            if events_to_send:
                                self.put_log_events(log_group, log_stream, events_to_send)
                                last_flush = now
                                logger.debug(f"Flushed {len(events_to_send)} events to CloudWatch {log_group}/{log_stream}")
                        except Exception as e:
            ...
            if cloudwatch_buffer and log_group and log_stream:
                try:
                    ...
                    logger.debug(f"Final flush of {len(cloudwatch_buffer)} events to CloudWatch")
                except Exception as e:
            ...
            logger.error(f"Error streaming logs from {container_id}: {e}", exc_info=True)
        finally:
            logger.warning(f'Container {container_id} logging is stopping, container status: {container.status}')
            # Cleanup
    ...
    def associate_request(self, container_name, container_id, request_id):
        ...
        with self._lock("LogManager.associate_request"):
            ...
            logger.debug(f"Associated request {C.CYAN}{request_id}{C.RESET} with container {container_name}:{container_id}")
    ...
    def disassociate_request(self, container_id):
        ...
        with self._lock("LogManager.disassociate_request"):
            ...
            if request_id:
                logger.debug(f"Disassociated request {C.CYAN}{request_id}{C.RESET} from container {container_id}")
            return request_id
    ...
    def get_logs_with_report(self, request_id, duration_ms=0, init_duration_ms=None, memory_size_mb=128, max_memory_mb=None):
    ...
    def container_logs(self, request_id, duration_ms, init_duration_ms=None):
        ...
        logger.debug(f"Finalizing logs for RequestId:{C.CYAN}{request_id}{C.RESET} duration:{duration_ms}ms init:{init_duration_ms}ms")

        if not log_group or not log_stream:
            logger.warning(f"No CloudWatch log stream found for request {C.CYAN}{request_id}{C.RESET}")
            return
        ...
        try:
            ...
            logger.debug(f"Added REPORT line to CloudWatch for {C.CYAN}{request_id}{C.RESET}")
        except Exception as e:
    ...
    def _cleanup_old_logs(self):
        ...
        """
        cutoff_time = datetime.now(timezone.utc).timestamp() - self.log_retention_seconds
        ...
        with self._lock("LogManager._cleanup_old_logs"):
            ...
            if request_ids_to_remove:
                logger.info(f"Cleaned up logs for {len(request_ids_to_remove)} old invocations")
    ...
    def put_log_events(self, group_name, stream_name, events):
        ...
        try:
            resp_json_or_token = self._forward_logs_to_aws_api(group_name, stream_name, events)

            # If forward succeeded and returned a token/json, return that.
            if resp_json_or_token:
                logger.info(f"Forwarded {len(events)} events to aws_api for {group_name}/{stream_name}")
                return resp_json_or_token

            # Otherwise, fallback to local storage
            logger.warning(f"Forward to aws_api failed for {group_name}/{stream_name}, falling back to local DB")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"{len(events)} events stored locally for {group_name}/{stream_name}")
            return next_token
        except Exception as e:
            logger.error(f"Failed to store logs locally for {group_name}/{stream_name}: {e}")
            raise
    ...
    def _forward_logs_to_aws_api(self, group_name, stream_name, events):
        ...
        try:
            ...
            try:
                logpath = os.path.join(os.getenv('STORAGE_PATH', '/data'), 'aws_api_forward.log')
                with open(logpath, 'a') as f:
                    f.write(f"{datetime.now(timezone.utc).isoformat()} POST -> {AWS_API_ENDPOINT} status={resp.status_code} group={group_name} stream={stream_name} events={len(events)}\n")
            except Exception:
                pass
            ...
            if resp.status_code in [200, 201]:
                ...
                logger.warning(f"AWS API returned {resp.status_code} for PutLogEvents: {resp.text[:200]}")
                return False
    ...
    def write_container_log_to_cloudwatch(self, log_group, log_stream, message, timestamp=None):
        """
    ...
    def describe_log_streams(self, log_group, log_stream_name_prefix=None, limit=50):
        ...
        if log_stream_name_prefix:
            streams = [s for s in streams if s['logStreamName'].startswith(log_stream_name_prefix)]

        return streams[:limit]

    def filter_log_events(self, log_group, log_stream_names=None, start_time=None,
                        end_time=None, filter_pattern=None, limit=100):
        """
        ...
        with self._lock("LogManager.filter_log_events"):
            ...
            streams_to_search = log_stream_names or list(self.log_groups[log_group].keys())
    ...
    def associate_container_with_log_stream(self, container_id, log_group, log_stream):
        ...
        with self._lock("LogManager.associate_container_with_log_stream"):
            ...
            logger.debug(f"Associated container {container_id} with {log_group}/{log_stream}")
    ...
    def set_active_request(self, container_id: str, request_id: str):
        ...
        with self._lock('LogManager.set_active_request'):
            ...
            logger.debug(f"Set active request for container {container_id}: {request_id}")
    ...
    def clear_active_request(self, container_id: str):
        ...
        with self._lock('LogManager.clear_active_request'):
            ...
            if request_id:
                logger.debug(f"Cleared active request for container {container_id}: {request_id}")
            return request_id
